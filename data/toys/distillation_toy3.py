...
def generate_(
    ...
    # for i in range(int(len_/max_batch_size)+1):
        ...
        for sample in data[i*max_batch_size:(i+1)*max_batch_size]:
            ...
            input_list.append([
                ...
                {"role": "system", "content": "Please respond in one short sentence. "
                                              ...
                                              "The sentence should describe the item's audience, for example: 'The item's audience is xxx.'"
                },
                {"role": "user", "content": "A user bought an item and said \"{explanation}\". "
                                            "\"{user_personality}\"."
                                            "\"{user_preference}\". \"{item_attribution}\". "
                                            "Use one short sentence to describe the item's audience."
                                            "The sentence should start with 'The item's audience is'. "
                                            .format(explanation=sample['explanation'], user_personality=sample['user_personality'], user_preference=sample['user_preference'], item_attribution=sample['item_attribution']),
                },
            ])
        ...
        for j, (dialog, result) in enumerate(zip(dialogs, results)):
            ...
            try:
                index0 = strs.index('The item')
                index1 = strs.index('.')
                #index2 = strs.index('The item')
                #print(strs[0:index1])
                #print(strs[index2:])
                data[i * max_batch_size + j]['item_audience'] = strs[index0:index1+1]
...
def main(
    ckpt_dir: str = '/public/home/hpc234712175/RDRec4/llama/Meta-Llama-3-8B-Instruct/',
    tokenizer_path: str = '/public/home/hpc234712175/RDRec4/llama/Meta-Llama-3-8B-Instruct/tokenizer.model',
    max_seq_len: int = 512,
    ...
    f = open('./data/toys/explanation_rationale2.json', 'r')
    data = json.load(f)
    f.close()
    ...
    # 保存回JSON文件
    with open('./data/toys/explanation_rationale3.json', 'w') as file:

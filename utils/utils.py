...
from model.templates import exp_templates, seq_templates, topn_templates, rea_templates, rs_templates
...
class RSSampler:
    ...
    def sample(self, num):
        ...
        for _ in range(num):
            ...
            #inputs.append(rs_templates[0].format(record['user'], record['item'], record['explanation'], record['user_personality'], record['user_preference'], record['item_attribution'], record['item_audience']))
            inputs.append(rs_templates[0].format(record['user'], record['item']))
...
class AllBatchify:
    ...
    def __init__(self, exp_data, user2items_pos, negative_num, item_num, tokenizer, exp_len, batch_size, ratio):
        ...
        self.rs_sampler = RSSampler(exp_data)
        ...
        self.rs_num = int(ratio[2] / sum(ratio) * batch_size)
        self.topn_num = batch_size - self.exp_num - self.seq_num - self.rs_num
        if self.exp_num > 0:
            self.batch_num = int(self.exp_sampler.sample_num / self.exp_num)
        elif self.rs_num > 0:
            self.batch_num = int(self.rs_sampler.sample_num / self.rs_num)
        elif self.seq_num > 0:
    ...
    def next_batch(self):
        ...
        tasks, inputs, outputs = self.rs_sampler.sample(self.rs_num)
        task_list.extend(tasks)
        input_list.extend(inputs)
        output_list.extend(outputs)
...
class RSBatchify:
    def __init__(self, exp_data, tokenizer, exp_len, batch_size):
        ...
        input_list, output_list = [], []
        for x in exp_data:
            input_list.append(rs_templates[0].format(x['user'], x['item']))
